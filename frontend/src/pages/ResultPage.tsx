...
/**
 * Trang Kết quả kiểm tra đạo văn
 * - Đọc kết quả từ localStorage (được lưu bởi uploadDocument)
 * - Hiển thị tỉ lệ tương đồng, thông tin chi tiết, danh sách match
 */
    ...
    return (
      <Layout>
        <Content style={{ padding: '50px', maxWidth: 900, margin: '0 auto', textAlign: 'center' }}>
          <Empty description="Không có kết quả kiểm tra" style={{ marginTop: 100 }}>
    ...
    <Layout>
      <Content style={{ padding: '50px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <Row gutter={[24, 24]}>
          ...
          <Col xs={24} md={12}>
            <Card style={{ textAlign: 'center', height: '100%' }} bordered={false}>
              ...
              <div style={{ marginTop: 20 }}>
                <Tag color={getColor()} style={{ fontSize: 14, padding: '4px 16px' }}>
                  Mức độ: {getLevelText()}
                </Tag>
              </div>
            </Card>
          ...
          <Col xs={24} md={12}>
            <Card title="Thông tin" style={{ height: '100%' }}>
              ...
              <p><strong>Corpus:</strong> {result.corpus_size || 'N/A'} tài liệu</p>
                  ...
                  {match.matched_segments && match.matched_segments.length > 0 && (
                    <div style={{ marginTop: 12, marginLeft: 70 }}>
                      <details style={{ cursor: 'pointer' }}>
                        ...
                        <div style={{ background: '#fafafa', padding: 16, borderRadius: 8, maxHeight: 300, overflowY: 'auto' }}>
                          {match.matched_segments.map((seg, segIdx) => (
                            <div key={segIdx} style={{ marginBottom: 16, padding: 12, background: 'white', borderRadius: 6, border: '1px solid #f0f0f0' }}>
                              <Tag color="blue" style={{ marginBottom: 4 }}>Đoạn {segIdx + 1}</Tag>
                              <div style={{ display: 'flex', gap: 16 }}>
                                <div style={{ flex: 1 }}>
                                  <Text strong style={{ color: '#ff4d4f', display: 'block', marginBottom: 4 }}>File của bạn:</Text>
                                  <Text style={{ background: '#fff2f0', padding: '4px 8px', borderRadius: 4, display: 'block', fontSize: 13 }}>
                                    "{seg.query_text}"
                                  </Text>
                                </div>
                                <div style={{ flex: 1 }}>
                                  <Text strong style={{ color: '#52c41a', display: 'block', marginBottom: 4 }}>Nguồn trùng khớp:</Text>
                                  <Text style={{ background: '#f6ffed', padding: '4px 8px', borderRadius: 4, display: 'block', fontSize: 13 }}>
                                    "{seg.source_text}"
                                  </Text>
                                </div>
                              </div>
        ...
        <div style={{ marginTop: 40, textAlign: 'center' }}>
          <Button type="primary" icon={<UploadOutlined />} size="large" onClick={() => navigate('/upload')} style={{ marginRight: 16 }}>
